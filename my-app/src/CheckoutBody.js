    ...
    render() {
        ...
        return (
            <div className="item-list">
                <div className={this.state.clicked ? "checkout-hide" : "cent"}>
                    {checkoutItems}
                    <h3>Total Price: {this.state.totalPrice.toLocaleString
                    ("en-US", { style: "currency", currency: "GBP"})}</h3>
                    <button className="confirm-purchase" onClick={() => this.handleClick()}>Confirm Purchase</button>
                </div>
                <div className={this.state.clicked ? "cent" : "checkout-hide"}>
                ...
                <br />
                <Link to="/">
                    <button className="back-to-main-page">Go back to shopping page</button>
                </Link>
            </div>
        )
    }
